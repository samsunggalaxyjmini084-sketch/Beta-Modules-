...
class PinChatMod(loader.Module):
    ...
    Максимально автономный, без лишних зависимостей.
    """

    strings = {
        "name": "PinChat",
        "_cls_doc": "Модуль для закрепления и открепления чатов в вашем списке чатов. Максимально автономный, без лишних зависимостей.",
    ...
    async def pchatcmd(self, message: Message):
        ...
        if cmd_prefix_end == -1: # Нет аргументов после команды
            ...
            raw_args_text = message.text[cmd_prefix_end:].strip()
            args = raw_args_text.split(None, 1) # Разделяем только первый аргумент
        ...
        if chat_id_str:
            try:
                ...
                await self._answer(message, self.strings("invalid_chat_id").format(chat_id_str))
                return
        ...
        if not target_entity:
            ...
            if not target_entity:
                await self._answer(message, self.strings("no_chat_specified"))
                return
        ...
        try:
            ...
            if pin_action:
                if current_pin_status:
                    await self._answer(message, self.strings("already_pinned").format(entity_name))
                    return
            else:
                if not current_pin_status:
                    await self._answer(message, self.strings("not_pinned").format(entity_name))
                    return
            ...
            if rpc_call_successful:
                if pin_action:
                    await self._answer(message, self.strings("pin_success").format(entity_name))
                else:
                    await self._answer(message, self.strings("unpin_success").format(entity_name))
            else:
