...
# 01101110 01100101 01110110 01100101 01110010 00100000 01110100 01100101 01101100 00100000 01100001 01101100 01101100 00100000 01100001 00100000 01101100 01101001 01100101 01100001 01110010 01101111 01110101 01101110 00100000 01100001 01101110 00100000 01101000 01110101 01110010 01110100 01111001 01101111 01110101
...
class TagAllMod(loader.Module):
    ...
    strings = {
        ...
        "_cfg_doc_autotagall_enabled_chats": "ID чата(ов), в которых включена автоматическая активация/остановка TagAll по триггерным сообщениям. Разделяйте запятыми. Например: <code>-100123456789, 123456789</code>",
    ...
    strings_de = {
        ...
        "_cfg_doc_autotagall_enabled_chats": "Chat-ID(s), in denen die automatische Aktivierung/Deaktivierung von TagAll durch Trigger-Nachrichten aktiviert ist. Durch Kommas getrennt eingeben. Zum Beispiel: <code>-100123456789, 123456789</code>",
    ...
    strings_tr = {
        ...
        "_cfg_doc_autotagall_enabled_chats": "TagAll'un tetikleyici mesajlarla otomatik olarak etkinleştirildiği/durdurulduğu sohbet kimliği(leri). Virgülle ayırın. Örnek: <code>-100123456789, 123456789</code>",
    ...
    strings_uz = {
        ...
        "_cfg_doc_autotagall_enabled_chats": "TagAll trigger xabarlari orqali avtomatik aktivlashtirilgan/to'xtatilgan chat ID(lar)i. Vergul bilan ajrating. Masalan: <code>-100123456789, 123456789</code>",
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            ...
            loader.ConfigValue(
                "autotagall_enabled_chats",
                "",
                lambda: self.strings("_cfg_doc_autotagall_enabled_chats"),
                validator=loader.validators.String(),
            ),
        )
    ...
    async def client_ready(self, client, db):
        ...
        if not self._message_watcher_handler:
            # Обновлен обработчик событий для отслеживания как входящих, так и исходящих сообщений.
            # Это позволяет модулю удалять свои собственные сообщения-триггеры.
            self._message_watcher_handler = self._client.add_event_handler(self._message_watcher, events.NewMessage(incoming=True, outgoing=True))
    ...
    def _get_allowed_chat_ids_map(self) -> dict[int, int]:
        ...
        Очищает строку от всего, кроме цифр и запятых, затем разбивает.
        """
    ...
    def _get_autotagall_enabled_chats(self) -> set[int]:
        """
        Парсит строку autotagall_enabled_chats из конфига в set[chat_id].
        """
        enabled_chats_raw = self.config["autotagall_enabled_chats"]
        enabled_chats_set = set()
        cleaned_enabled_chats_raw = re.sub(r"[^0-9,\-]", "", enabled_chats_raw) # Разрешаем минус для отрицательных ID чатов
        for chat_id_str in cleaned_enabled_chats_raw.split(','):
            chat_id_str = chat_id_str.strip()
            if chat_id_str:
                try:
                    chat_id = int(chat_id_str)
                    enabled_chats_set.add(chat_id)
                except ValueError:
                    logger.warning(f"Неверный ID чата в конфигурации 'autotagall_enabled_chats' после очистки: '{chat_id_str}'. Должен быть целым числом.")
        return enabled_chats_set
    ...
    async def _resolve_target_chat(self, message: Message, raw_args: str) -> tuple[int | None, str | None]:
        ...
        Возвращает None для effective_target_chat_id в случае ошибки или неразрешенного чата.
        """
        ...
        # 1. Попытка разобрать индекс чата из аргументов команды
        ...
        if chat_index_match:
            try:
                ...
                if chat_index in allowed_chats_map:
                    ...
                    # Уведомляем о перенаправлении, если чат изменился
                    ...
                    # Указан неверный индекс чата из списка разрешенных
                ...
                # Недействительный целочисленный индекс, игнорируем и продолжаем обычный разбор
                pass

        # 2. Если индекс не был предоставлен или был недействительным, применяем логику на основе текущего чата
        ...
        elif original_chat_id in allowed_chat_ids_set:
            # Команда запущена в разрешенном чате, выполняем в нем
            return original_chat_id, remaining_args
        else:
            # Команда запущена в неразрешенном чате
            if len(allowed_chat_ids_set) == 1:
                # Только один разрешенный чат, перенаправляем команду туда
    ...
    def _parse_trigger_string(self, trigger_raw: str) -> tuple[str, int | None]:
        """
        Парсит необработанную строку триггера на базовое сообщение (нижний регистр, без пробелов по краям)
        и необязательный индекс чата, указанный в скобках.
        Пример: 'стоп(1)' -> ('стоп', 1), 'запуск' -> ('запуск', None).
        """
            ...
            if "," in cleaned_timeout_str:
                ...
                if values:
                    if len(values) > 1 and event.last_timeout is not None and event.last_timeout in values:
                        # Избегаем повтора предыдущего таймаута, если есть другие варианты
                ...
                try:
                    ...
                    min_val = max(0.0, min_val) # Гарантируем неотрицательность
                    max_val = max(0.0, max_val) # Гарантируем неотрицательность

                    if min_val > max_val: # Если min > max, меняем местами
                ...
                try:
                    current_timeout = max(0.0, float(cleaned_timeout_str)) # Гарантируем неотрицательность
                except ValueError:
    ...
    async def _message_watcher(self, message: Message):
        """
        Отслеживает входящие и исходящие сообщения на предмет настроенных триггерных сообщений
        (остановка и запуск TagAll) и опциональных пользователей, которые могут их использовать.
        """
        # Игнорируем сообщения без текста или chat_id.
        ...
        # Глобальная проверка: если 'allowed_chat_ids' настроен, игнорируем сообщения из неразрешенных чатов.
        # Это применимо только к входящим сообщениям, исходящие сообщения юзербота (команды-триггеры)
        # могут быть обработаны и удалены, если они соответствуют условиям триггера.
        if allowed_chat_ids_set and chat_id not in allowed_chat_ids_set and message.incoming:
            return

        # Проверяем, включен ли autotagall для текущего чата
        autotagall_enabled_chats = self._get_autotagall_enabled_chats()
        is_autotagall_enabled_for_chat = chat_id in autotagall_enabled_chats

        if not is_autotagall_enabled_for_chat:
            return # Если autotagall не включен для этого чата, не обрабатываем триггеры

        current_tagall_event = self._tagall_events.get(chat_id)

        # --- Обработка триггера ОСТАНОВКИ ---
        # is_autotagall_enabled_for_chat заменяет проверку stop_triggers_enabled
        trigger_stop_messages_raw = self.config["trigger_message"]
        parsed_stop_triggers = []
        for t_raw in trigger_stop_messages_raw.split(','):
            if t_raw.strip(): # Игнорируем пустые строки после сплита
                parsed_stop_triggers.append(self._parse_trigger_string(t_raw))

        has_stop_trigger_message = False
        for base_trigger_text, config_chat_index in parsed_stop_triggers:
            if base_trigger_text in message_text_lower:
                # Проверяем, относится ли триггер к текущему чату
                if config_chat_index is None:
                    # Если индекс в конфигурации не указан (например, 'стоп'), триггер применяется ко всем
                    # чатам, для которых включен autotagall (уже проверено выше).
                    has_stop_trigger_message = True
                    break
                else:
                    # Если индекс указан (например, 'стоп(1)'), триггер сработает только в чате,
                    # ID которого соответствует этому индексу в allowed_chats_map.
        ...
        # Проверяем авторизацию пользователя/бота для остановки
        ...
        # Если список ID пуст, любой пользователь авторизован. Иначе - только указанные.
        is_authorized_stop_user = not trigger_stop_user_ids or (message.sender and message.sender.id in trigger_stop_user_ids)

        # Если TagAll запущен в этом чате, сообщение соответствует триггеру и пользователь авторизован, останавливаем.
        if current_tagall_event and current_tagall_event.state and has_stop_trigger_message and is_authorized_stop_user:
            ...
            # Если сообщение было отправлено самим юзерботом, удаляем его
            if message.out:
                with contextlib.suppress(Exception):
                    await message.delete()
            return # Предотвращаем обработку как триггера активации

        # --- Обработка триггера АКТИВАЦИИ ---
        # is_autotagall_enabled_for_chat заменяет проверку activation_triggers_enabled
        ...
        for base_trigger_text, config_chat_index in parsed_activation_triggers:
            if base_trigger_text in message_text_lower:
                # Проверяем, относится ли триггер к текущему чату (логика аналогична триггеру остановки)
                if config_chat_index is None:
        ...
        # Проверяем авторизацию пользователя/бота для активации
        ...
        # Если TagAll еще не запущен в этом чате, сообщение соответствует триггеру и пользователь авторизован, запускаем.
        if has_activation_trigger_message and is_authorized_activation_user:
            if current_tagall_event and current_tagall_event.state:
                logger.info(f"TagAll уже запущен в чате {chat_id}, игнорируем триггер активации '{message.text}'.")
                # Если сообщение было отправлено самим юзерботом, и TagAll уже запущен,
                # можно также удалить это сообщение для чистоты.
                if message.out:
                    with contextlib.suppress(Exception):
                        await message.delete()
                return
            ...
            # Запускаем процесс TagAll в фоновом режиме
            self._client.loop.create_task(self._run_tagall_process(chat_id, "", event, True))

            # Если сообщение было отправлено самим юзерботом, удаляем его после активации
            if message.out:
                with contextlib.suppress(Exception):
                    await message.delete()
    ...
    async def _run_tagall_process(self, chat_id: int, message_prefix: str, event: StopEvent, silent_start: bool = False):
        ...
        if is_bot_sender:
            try:
                # Проверка на наличие и доступность клиента инлайн-бота
                ...
                # Подавляем ошибки, если бот уже в чате или не может быть приглашен
                with contextlib.suppress(Exception):
        ...
        try:
            ...
            while self.config["cycle_tagging"] or first_pass:
                if not event.state: # Проверяем сигнал остановки
                    break

                # Проверка на общее время работы
                if self.config["duration"] > 0 and (time.time() - start_time) > self.config["duration"]:
                    logger.info(f"TagAll в чате {chat_id} завершен по истечении установленного времени ({self.config['duration']}с).")
                    event.stop()
                    break

                current_participants_for_cycle = []
                if self.config["cycle_tagging"] and not first_pass:
                    # Для циклических тегов, обновляем список участников в каждом цикле
                ...
                for chunk in utils.chunks(participants, self.config["chunk_size"]):
                    ...
                    # Проверка на общее время работы внутри цикла
                    if self.config["duration"] > 0 and (time.time() - start_time) > self.config["duration"]:
                        logger.info(f"TagAll в чате {chat_id} завершен по истечении установленного времени ({self.config['duration']}с).")
                        event.stop()
                        break
                ...
                first_pass = False # Первый проход завершен
                if self.config["cycle_tagging"] and event.state:
                    ...
                    break # Нециклический режим, выходим после одного прохода

        finally:
            if self.config["delete"]:
                with contextlib.suppress(Exception): # Подавляем ошибки при удалении сообщений
            ...
            if event.state: # Если процесс завершился естественным образом (не был остановлен вручную)
                logger.info(f"Процесс TagAll завершен естественным образом в чате {chat_id}.")
            else: # Если был остановлен вручную
                logger.info(f"Процесс TagAll был остановлен в чате {chat_id}.")
    ...
    async def stoptagall(self, message: Message):
        ...
        if event and event.state:
            logger.info(f"Команда .stoptagall использована в чате {message.chat_id} (целевой {target_chat_id}). Отправлен сигнал остановки.")
            event.stop()
        else:
    ...
    async def autotagall(self, message: Message):
        ...
        current_enabled_chats = self._get_autotagall_enabled_chats()

        if args == "on":
            if target_chat_id not in current_enabled_chats:
                current_enabled_chats.add(target_chat_id)
                self.config["autotagall_enabled_chats"] = ",".join(map(str, sorted(list(current_enabled_chats))))
                await utils.answer(message, self.strings("triggers_state_enabled").format(chat_id=target_chat_id))
                logger.info(f"Триггеры TagAll включены для чата {target_chat_id}.")
            else:
                await utils.answer(message, self.strings("triggers_status_enabled").format(chat_id=target_chat_id))
        elif args == "off":
            if target_chat_id in current_enabled_chats:
                current_enabled_chats.remove(target_chat_id)
                self.config["autotagall_enabled_chats"] = ",".join(map(str, sorted(list(current_enabled_chats))))
                await utils.answer(message, self.strings("triggers_state_disabled").format(chat_id=target_chat_id))
                logger.info(f"Триггеры TagAll выключены для чата {target_chat_id}.")
            else:
