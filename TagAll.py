...
class TagAllMod(loader.Module):
    ...
    async def _message_watcher(self, message: Message):
        """
        Отслеживает входящие сообщения на предмет настроенных триггерных сообщений (остановка и запуск TagAll)
        и опциональных пользователей, которые могут их использовать.
        """
        # Игнорируем исходящие сообщения, пустые сообщения или сообщения без chat_id
        ...
        # Получаем настройки триггеров для текущего чата
            ...
            for base_trigger_text, config_chat_index in parsed_stop_triggers:
                if base_trigger_text in message_text_lower:
                    ...
                    if config_chat_index is None:
                        # Если индекс в конфигурации не указан (например, 'стоп'), триггер применяется ко всем разрешенным чатам.
                        # Глобальная проверка 'allowed_chats_set' уже была выполнена выше.
