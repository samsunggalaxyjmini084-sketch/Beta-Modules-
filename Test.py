...
class Gemini(loader.Module):
    ...
    async def gclear(self, message: Message):
        ...
        if hist_key in self.conversations:
            ...
            keys_to_del =[k for k in self.pager_cache if self.pager_cache[k].get("chat_id") == chat_id]
    ...
    async def gmemdel(self, message: Message):
        """[N] [start/end] — удалить N пар сообщений из памяти (по умолчанию с конца)."""
        args = utils.get_args_raw(message).lower().split()
        n = 1  # Default to 1 pair
        delete_from_start = False

        if not args:
            # No args, delete 1 pair from end (default)
            pass
        elif len(args) == 1:
            try:
                n = int(args[0])
                ...
                if args[0] == "start":
                    ...
                    delete_from_start = False  # Explicitly end
                else:
                    return await utils.answer(message, "Неверный аргумент. Используйте <code>.gmemdel [N] [start/end]</code>")
        elif len(args) == 2:
            try:
                n = int(args[0])
            except ValueError:
                return await utils.answer(message, "Неверное число N. Используйте <code>.gmemdel [N] [start/end]</code>")
            ...
            return await utils.answer(message, "Слишком много аргументов. Используйте <code>.gmemdel [N] [start/end]</code>")

        if n <= 0:
            return await utils.answer(message, "N должно быть положительным числом.")

        cid = "global_context" if self.config["global_memory"] else utils.get_chat_id(message)
        hist = self._get_structured_history(cid)

        num_entries_to_delete = n * 2 # Each "pair" consists of a user message and a model response

        if num_entries_to_delete > len(hist):
            return await utils.answer(message, f"Недостаточно истории ({len(hist)//2} пар) для удаления {n} пар сообщений.")

        if delete_from_start:
            self.conversations[str(cid)] = hist[num_entries_to_delete:]
            msg_text = f"🧹 Удалено первых <b>{n}</b> пар сообщений из памяти."
        else:
            self.conversations[str(cid)] = hist[:-num_entries_to_delete]
            msg_text = f"🧹 Удалено последних <b>{n}</b> пар сообщений из памяти."
